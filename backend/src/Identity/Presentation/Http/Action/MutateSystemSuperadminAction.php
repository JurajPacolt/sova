...
final readonly class MutateSystemSuperadminAction
{
    ...
    public function __invoke(
        ...
        $this->authorization->require(
            ...
            Permission::SystemSuperadminsManage,
            AuthorizationScope::system(),
        );
        $userId = $this->userId($args['userId'] ?? '');
        $requestId = $this->requestId($request);
        $ipAddress = $this->ipAddress($request);
        $user = match ($request->getMethod()) {
            'PUT' => $this->administration->grantSuperadmin(
                $userId,
                $session->actorUserId,
                $requestId,
                $ipAddress,
            ),
            'DELETE' => $this->administration->revokeSuperadmin(
                $userId,
                $session->actorUserId,
                $requestId,
                $ipAddress,
            ...
            default => throw new RuntimeException(
                'Unsupported system superadmin mutation.',
            ),
        };
    ...
    private function session(ServerRequestInterface $request): SessionContext
    {
        ...
        if (!$session instanceof SessionContext) {
            throw new RuntimeException(
                'System superadmin administration requires a session context.',
            );
        }
