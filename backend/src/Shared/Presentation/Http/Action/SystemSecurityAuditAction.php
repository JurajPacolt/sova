...
final readonly class SystemSecurityAuditAction
{
    ...
    public function __invoke(
        ...
        $this->authorization->require(
            ...
            Permission::SystemAuditView,
            AuthorizationScope::system(),
        );
        $query = $this->validator->validate($request->getQueryParams());
        $page = $this->reader->page($query, null);
        $this->audit->record(
            eventType: 'SYSTEM_AUDIT_VIEWED',
            outcome: 'SUCCESS',
            reasonCode: 'SYSTEM_AUDIT_VIEWED',
    ...
    private function session(ServerRequestInterface $request): SessionContext
    {
        ...
        if (!$session instanceof SessionContext) {
            throw new RuntimeException(
                'System audit access requires a session context.',
            );
        }
