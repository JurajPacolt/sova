...
final class InvitationApiTest extends TestCase
{
    ...
    public function testInvitationListAndResendRotateTheOnlyUsableToken(): void
    {
        $login = $this->login(
            'superadmin-invite@example.test',
            'Superadmin invitation passphrase',
        ...
        $listed = $this->authenticatedRequest(
            $login,
            'GET',
            sprintf('/api/v1/tenants/%s/invitations', $this->tenantId),
        );
        self::assertSame(200, $listed->getStatusCode());
        $invitations = $this->decode($listed)['invitations'] ?? null;
        self::assertIsArray($invitations);
        self::assertCount(1, $invitations);
        $listedInvitation = $invitations[0] ?? null;
        self::assertIsArray($listedInvitation);
        self::assertSame(
            'managed-invite@example.test',
            $listedInvitation['email'] ?? null,
        );
        self::assertSame('PENDING', $listedInvitation['status'] ?? null);

        $rateLimited = $this->authenticatedRequest(
            $login,
            'POST',
            sprintf(
                '/api/v1/tenants/%s/invitations/%s/resend',
                $this->tenantId,
                $invitationId,
            ),
        );
        ...
        self::assertSame(
            ...
            $this->decode($rateLimited)['code'] ?? null,
        );

        $firstMailer = new CapturingInvitationMailer();
        self::assertSame(1, $this->invitationWorker($firstMailer)->runBatch());
        $firstToken = $firstMailer->messages[0]['token'];
        ...
        $resent = $this->authenticatedRequest(
            $login,
            'POST',
            sprintf(
                '/api/v1/tenants/%s/invitations/%s/resend',
        ...
        self::assertSame(200, $resent->getStatusCode());
        self::assertNotSame(
            $firstHash,
            $this->connection->fetchOne(
                'SELECT token_hash FROM tenant_invitations WHERE id = :id',
        ...
        $secondMailer = new CapturingInvitationMailer();
        self::assertSame(1, $this->invitationWorker($secondMailer)->runBatch());
        self::assertCount(1, $secondMailer->messages);
        self::assertNotSame(
            $firstToken,
            $secondMailer->messages[0]['token'],
        );
        self::assertSame(
            1,
            $this->connection->fetchOne(
                <<<'SQL'
                    ...
                    FROM security_audit_events
                    WHERE tenant_id = :tenant_id
                        AND event_type = 'TENANT_INVITATION_RESENT'
                        AND metadata->>'invitation_id' = :invitation_id
                    SQL,
                [
                    'tenant_id' => $this->tenantId,
                    'invitation_id' => $invitationId,
    ...
    public function testPendingInvitationExpiryCanChangeAndRevocationPurgesDelivery(): void
    {
        $login = $this->login(
            'superadmin-invite@example.test',
            'Superadmin invitation passphrase',
        ...
        $payload = $this->decode($created)['invitation'] ?? null;
        self::assertIsArray($payload);
        $invitationId = $payload['id'] ?? null;
        self::assertIsString($invitationId);
        $newExpiry = (new \DateTimeImmutable('+2 days'))->format(DATE_ATOM);

        $changed = $this->authenticatedRequest(
            $login,
            'PATCH',
            sprintf(
                '/api/v1/tenants/%s/invitations/%s',
                $this->tenantId,
                $invitationId,
            ),
        ...
        $changedInvitation = $this->decode($changed)['invitation'] ?? null;
        self::assertIsArray($changedInvitation);
        $changedExpiry = $changedInvitation['expires_at'] ?? null;
        self::assertIsString($changedExpiry);
        self::assertSame(
            (new \DateTimeImmutable($newExpiry))->getTimestamp(),
            (new \DateTimeImmutable($changedExpiry))->getTimestamp(),
        );

        $revoked = $this->authenticatedRequest(
            $login,
            'DELETE',
            sprintf(
                '/api/v1/tenants/%s/invitations/%s',
        ...
        $revokedInvitation = $this->decode($revoked)['invitation'] ?? null;
        self::assertIsArray($revokedInvitation);
        self::assertSame(
            'REVOKED',
            $revokedInvitation['status'] ?? null,
        );
        self::assertSame(
            0,
            $this->connection->fetchOne(
                <<<'SQL'
                    ...
                    FROM outbox_sensitive_payloads sensitive
                    INNER JOIN outbox_events event ON event.id = sensitive.event_id
                    WHERE event.aggregate_type = 'TENANT_INVITATION'
                        AND event.aggregate_id = :invitation_id
                    SQL,
                ['invitation_id' => $invitationId],
            ),
        );
